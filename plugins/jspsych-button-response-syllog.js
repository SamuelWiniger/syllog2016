/**
 * jspsych-button-response (adapted)
 * Josh de Leeuw
 *
...
jsPsych.plugins["button-response-syllog"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      ...
      $('#jspsych-button-response-btngroup').append(
        $(str).attr('id', 'jspsych-button-response-button-' + i).data('choice', i).addClass('jspsych-button-response-button').on('click', function(e) {
          var tempChoice = $('#' + this.id).data('choice');
          after_response(tempChoice);
        })
      );
    }
    ...
    function after_response(tempChoice){

      // measure rt
      var end_time = Date.now();
      var rt = end_time - start_time;
      if(tempChoice == 0){response.button = 'yes';}
      else if (tempChoice == 1) {response.button = "I dont know";}
